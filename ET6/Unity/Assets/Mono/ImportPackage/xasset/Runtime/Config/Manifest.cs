    ...
    public class Manifest : ScriptableObject
    {
        ...
        private void OnLoad()
        {
            ...
            foreach (var bundle in bundles)
            {
                ...
                {
                    //修改：去掉xasset打包后AB包名中的哈希值
                    bundle.nameWithAppendHash = $"{bundle.name}";//bundle.nameWithAppendHash = $"{bundle.name}_{bundle.hash}";
                }
                else
                {
                    var fileNameWithoutExtension = Path.GetFileNameWithoutExtension(bundle.name);
                    //修改：去掉xasset打包后AB包名中的哈希值
                    bundle.nameWithAppendHash = $"{fileNameWithoutExtension}{bundleExtension}";//$"{fileNameWithoutExtension}_{bundle.hash}{bundleExtension}";
                }
            }
