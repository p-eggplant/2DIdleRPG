...
namespace ET
{
    [FriendClass(typeof(UIExample))]
    // 告诉系统 UI的id
    [UIEventAttribute(nameof(UIExample))]

    // 组件名+UIEvent 定义你的UI事件处理类
    public class UIExampleUIEvent : UIEventBase<UIExample>
    {
        ...
        public override void OnCreateWindow(ref UIComponent pUIComponent)
        {
            // 系统启动时回调CreateUI
            // 1. 往UI上挂在自己写的窗口组件
            pUIComponent.AddComponent<UIExample>();

        }

        public override void OnLoadResouse(UIExample self, string szWindowName, Dictionary<EUILayer, Transform> dicLayer)
        {
            // 当AB被加载后，会调用LoadUI
            self.Domain.GetComponent<ResourcesLoaderComponent>().Load("UILogin.unity3d");
            GameObject pUIGmMain = (GameObject)ResourcesComponent.Instance.GetAsset("UILogin.unity3d", "UIExample");
        ...
        public override void OnDestroyResouse(UIExample self)
        {
        ...
        public override void OnShowWindow(UIExample self, object showData)
        {
            // 窗口被显示的时候，会调用ShowUI
            // 在这里处理界面打开的刷新
        }

        public override void OnHideWindow(UIExample self)
        {
            // 界面被关闭的时候，会调用HideUI
            // 关闭掉一些东西，例如定时器
        }
