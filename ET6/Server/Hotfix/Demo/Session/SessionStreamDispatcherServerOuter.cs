            ...
            switch (message)
            {
                ...
                case IActorRequest actorRequest:  // 分发IActorRequest消息，目前没有用到，需要的自己添加
                {
                    ...
                    if (sessionPlayerComponent == null)
                        return;
                    long unitId = sessionPlayerComponent.PlayerInstanceId;
                    int rpcId = actorRequest.RpcId; // 这里要保存客户端的rpcId
                    long instanceId = session.InstanceId;
                    IResponse response = await ActorMessageSenderComponent.Instance.Call(unitId, actorRequest);
                    response.RpcId = rpcId;
                    // session可能已经断开了，所以这里需要判断
                    if (session.InstanceId == instanceId)
                    {
                ...
                case IActorMessage actorMessage:  // 分发IActorMessage消息，目前没有用到，需要的自己添加
                {
                    ...
                    if (sessionPlayerComponent == null)
                        return;
                    long unitId = sessionPlayerComponent.PlayerInstanceId;
                    ActorMessageSenderComponent.Instance.Send(unitId, actorMessage);
                    break;
                }
